...
public class FamilyTree {
	private static class TreeNode<T> {
		...
		// with the given data. Returns the node, or null if not found.
		TreeNode getNodeWithData(T targetData) {
			// Does this node have the target data?
			if (data.equals(targetData)) return this;

			// No, recurse. Check all children of this node.
			for (TreeNode child : children) {
				// If child.getNodeWithData(targetName) returns a non-null node,
				// then that's the node we're looking for. Return it.
				TreeNode result = child.getNodeWithData(targetData);
		...
		// parent and ending with the root. Order is from recent to ancient.
	...
	private void addLine(String line) throws TreeException {
		...
		if (root == null)
			...
			parentNode = root.getNodeWithData(parent);
		...
		for (String child : childrenArray) parentNode.addChild(new TreeNode<String>(child));
	}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException {
		...
		TreeNode<String> node1 = root.getNodeWithData(name1);
		if (node1 == null)
			// Throw a TreeException with a useful message
			throw new TreeException("Child "+name1+" does not exist.");
		// node whose name is name2
		TreeNode<String> node2 = root.getNodeWithData(name2);
		if (node2 == null)
